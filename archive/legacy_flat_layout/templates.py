...
The CSS / JS referenced from the templates lives in :mod:`patentdoc_conv.assets`
and is copied to ``HTML/assets/`` at build time.
"""
...
def text_viewer_html(data: dict, asset_prefix: str) -> str:
    ...
    ``asset_prefix`` is ``"../assets"`` for files placed in ``HTML/text/`` and
    ``"assets"`` for ``HTML/index_text.html``.
    """
